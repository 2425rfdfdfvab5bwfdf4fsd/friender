...
if not defined _ARIX_BRIDGE_LAUNCHED (
    set _ARIX_BRIDGE_LAUNCHED=1
    cmd /k "%~f0"
    exit /b
)
...
echo  Press Ctrl+C to disconnect the bridge.
echo.

python local_bridge/bridge_agent.py --server %BRIDGE_URL%

echo.
echo  Bridge disconnected. Press any key to close.
pause
