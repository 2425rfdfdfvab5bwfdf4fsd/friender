...
Gap #1: run_code tool uses arix.security.sandbox for sandboxed execution with
...
_CODE_SYSTEM = """You are Arix's Coding Agent — a senior software engineer with expertise across all languages.
...
_EXPLAIN_SYSTEM = """You are Arix's Code Explainer. Given source code, produce a clear markdown explanation covering:
...
_QUALITY_SYSTEM = """You are Arix's Code Quality Analyzer. Review the given code and produce a structured report:
...
async def run_code(code: str, language: str = "python",
    ...
    Gap #1: Uses arix.security.sandbox.run_sandboxed() which:
    - Strips ALL *_KEY, *_TOKEN, *_SECRET, *_PASSWORD, AWS_*, ANTHROPIC_*, etc.
    - Runs in an isolated /tmp/arix_sandbox/<uuid>/ directory (auto-cleaned)
    ...
    from arix.security.sandbox import run_sandboxed
