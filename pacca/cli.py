"""PACCA command-line interface.

Usage:
    pacca serve          Start the PACCA web server (default)
    pacca doctor         Run system health checks
    pacca init           Interactive first-run setup wizard
    pacca version        Print version and exit
"""
...
def cmd_version() -> None:
    print(f"PACCA v{VERSION}")


def cmd_doctor() -> None:
    print(f"\nPACCA v{VERSION} — System Health Check")
    print("=" * 50)
    errors = 0
    ...
    # Config file
    cfg_path = PACCA_DIR / "config.json"
    if cfg_path.exists():
        ...
        if not ok:
            errors += 1
            _print("  → fix: chmod 600 ~/.pacca/config.json", ok=None)
    else:
        _print(f"Config file not found — will be created on first run ({cfg_path})", ok=None)

    # Audit log
    audit_path = PACCA_DIR / "audit.log"
    ...
    # PACCA_ADMIN_TOKEN
    if os.environ.get("PACCA_ADMIN_TOKEN"):
        _print("PACCA_ADMIN_TOKEN set — API authentication enabled", ok=True)
    else:
        _print("PACCA_ADMIN_TOKEN not set — API is unauthenticated (fine for local use)", ok=None)
...
def cmd_init() -> None:
    print(f"\nPACCA v{VERSION} — First-Run Setup")
    print("=" * 50)
    PACCA_DIR.mkdir(parents=True, exist_ok=True)

    from pacca.config import PACCAConfig, CONFIG_FILE
    cfg = PACCAConfig.load()
    ...
    print("     Run 'pacca doctor' to verify your setup.")
    print("     Run 'pacca serve' to start PACCA.\n")
...
def main() -> None:
    ...
    if cmd in ("version", "--version", "-v"):
        ...
        print("Usage: pacca [serve|doctor|init|version]")
        sys.exit(1)
